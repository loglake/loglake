...
The staged 0.10.1 candidate lives in `bench/fork-rebase/`; it is a separate
workspace and cannot affect production dependency resolution. Slice 1 replaces
the candidate copies of the local schema-update and snapshot-expiry actions
with upstream actions plus caller adapters. The shipping 0.9.1 forks retain
both local actions until the final adoption slice.
