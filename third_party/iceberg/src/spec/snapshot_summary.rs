...
fn get_prop(previous_summary: &Summary, prop: &str) -> Result<i64> {
    let value_str = previous_summary
        ...
        .map(String::as_str)
        .unwrap_or("0");
    // These are row/file/byte totals — `total-file-size` alone exceeds i32 for a
    // multi-GB table, so parse as i64 (the previous i32 panicked on large tables).
...
fn update_totals(
    ...
    if let Some(value) = summary
        ...
        .map(|value| value.parse::<u64>().unwrap())
    {
        // Saturating: a VALID commit never removes more than `prev + added` (the
        // removed files are live and counted in `prev`). An INVALID overwrite that
        // tries to remove already-gone files (e.g. a re-cluster replayed against
        // stale handles) would underflow here — saturate instead of panicking; the
        // manifest rewrite's live-file validation rejects that commit regardless,
        // so the bogus total is never persisted.
