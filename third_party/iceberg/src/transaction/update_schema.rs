...
impl AddColumn {
    ...
    /// Make this addition a no-op when the target column already exists.
    ///
    /// This does not compare the existing column's type or nullability. Callers
    /// must validate those independently before requesting idempotent replay.
...
impl TransactionAction for UpdateSchemaAction {
    async fn commit(self: Arc<Self>, table: &Table) -> Result<ActionCommit> {
        ...
        if additions_by_parent.is_empty() && delete_ids.is_empty() {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_add_column_if_not_exists_is_a_noop() {
        let table = make_v2_table();
        let tx = Transaction::new(&table);

        let action = tx.update_schema().add_column(
            AddColumn::optional("x", Type::Primitive(PrimitiveType::Int)).if_not_exists(),
        );
