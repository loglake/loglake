...
All three inputs are wired: the chart renders `LOGLAKE_CATALOG_URI` from
`postgres.existingSecret` and `postgres.secretKeys`, and `LOGLAKE_WAREHOUSE_URL`
from `s3.bucket` and `s3.warehousePrefix`, on every component, and
`wal.mirror.prefix` as `LOGLAKE_WAL_MIRROR_PREFIX` on the compactor in both
